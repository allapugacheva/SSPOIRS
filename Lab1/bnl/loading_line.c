...
int refresh_lline(LLINE* lline, double new_percent, double time) {
    ...
    printf("%%");
    char* speedString = get_speed_stirng(get_speed(lline, -lline->prev_percent + new_percent, time));
    ...
    lline->clear_size = 2 + lline->amount_stages + 2 + 8 + strlen(speedString);
...
double get_speed(LLINE* lline, double percent_up, double time) {
    return (percent_up * lline->fileSize) / time / 100.0;
}
